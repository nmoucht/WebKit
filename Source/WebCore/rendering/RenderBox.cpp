...
void RenderBox::styleDidChange(StyleDifference diff, const RenderStyle* oldStyle)
{
    ...
    if (oldStyle && oldStyle->scrollbarColor() != newStyle.scrollbarColor()) {
        std::pair<std::optional<Color>, std::optional<Color>> scrollbarColor;

        if (auto value = newStyle.scrollbarColor().tryValue())
            scrollbarColor = { newStyle.colorResolvingCurrentColor(value->thumb), newStyle.colorResolvingCurrentColor(value->track) };
